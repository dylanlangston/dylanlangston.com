<script lang="ts">
	...
	const generationConfig = {
		...
		stopSequences: ['Ensure the topic stays on Dylan Langston.'],
	...
	async function sendMessage() {
		...
		messages = [...messages, { text: text, user: 'user' }];
		...
		try
		{
			...
			messages = [...messages, { text: response.response.text(), user: 'bot' }];
		}
		catch (er) {
			messages = [...messages, { text: "An error occured! Please try again.", user: 'error' }];
			console.error(er);
		}
	...
	onMount(() => {
		// Welcome message from the bot when component mounts
		messages = [{ text: 'Welcome! How can I assist you?', user: 'bot' }];
	});
			...
			<div class="block mt-2 ml-2">
				<Ripple color={'currentColor'}>
					<button
						...
						class="w-full px-10 py-2 font-medium rounded-lg duration-150 hover:shadow-md hover:bg-rainbow disabled:bg-transparent transition-colors disabled:cursor-not-allowed"
					>
						Send
					</button>
				</Ripple>
