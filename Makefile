...
release-docker:  ## Builds Web Version for publish using docker.
	@docker build --cache-from dylanlangston.com:build --cache-from debian:stable-slim -t dylanlangston.com:build --target publish . --build-arg VERSION=$(VERSION) --build-arg OPTIMIZE=$(OPTIMIZE)
