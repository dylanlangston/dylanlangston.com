...
ifeq ($(USE_NODE),1)
	@if [ ! -d ~/.cache/ms-playwright/chromium* ] && \
	   [ ! -d ~/.cache/ms-playwright/ffmpeg* ] && \
	   [ ! -d ~/.cache/ms-playwright/firefox* ] && \
	   [ ! -d ~/.cache/ms-playwright/webkit* ]; then \
...
release-docker:  ## Builds Web Version for publish using docker.
	@docker build --rm --progress=plain  --network=host -t dylanlangston.com . --target publish --output type=local,dest=$(OUTPUT_DIR) --build-arg VERSION=$(VERSION) --build-arg OPTIMIZE=$(OPTIMIZE) --build-arg PRECOMPRESS_RELEASE=$(PRECOMPRESS_RELEASE)
